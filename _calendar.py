...
class Calendar(HTMLCalendar):
        ...
        if day and day in self.day_entries:
            ...
            archive_day_url = reverse('zinnia:entry_archive_day',
                                      args=[day_date.strftime('%Y'),
                                            day_date.strftime('%m'),
                                            day_date.strftime('%d')])
            return '<div class="%s entry col"><a href="%s" '\
                   'class="archives">%d</a></div>' % (
                       self.cssclasses[weekday], archive_day_url, day)

        if day == 0:
            return '<div class="noday weekday col">&nbsp;</div>' # day outside month
        else:
            return '<div class="%s weekday col">%d</div>' % (self.cssclasses[weekday], day)
    ...
    def formatweekday(self, day):
        ...
        """
        return '<div class="%s weekday col">%s</div>' % (self.cssclasses[day],
    ...
    def formatweekheader(self):
        ...
        """
        s = ''.join(self.formatweekday(i) for i in super(Calendar, self).iterweekdays())
        return '<div class="header row">%s</div>' % s
    ...
    def formatweek(self, theweek):
        ...
        return '<div class="week row">%s</div>' % s
